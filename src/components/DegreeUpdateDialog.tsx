...
const DegreeUpdateDialog = ({
  open,
  onClose,
  data,
}: DegreeUpdateDialogProps) => {
  if (!data) return null;
  ...
  const handleSave = async (
    ...
    try {
      ...
      const result = validateDegreeInfo(updatedDegree);
      if (!result.success) {
          ...
          <div style={{ display: "flex", gap: 32 }}>
            ...
            <div className="flex w-full flex-col gap-3">
              ...
              <Typography>
                Giời cập nhật:{" "}
                {degree?.updatedAt
                  ? new Date(degree.updatedAt).toLocaleString("vi-VN")
                  : ""}
              </Typography>
            </div>
          </div>
