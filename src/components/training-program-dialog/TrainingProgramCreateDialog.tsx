...
import type { TrainingProgramReq, TrainingProgramRequest } from '../../types/TrainingProgram';
...
const TrainingProgramCreateDialog: React.FC<TrainingProgramCreateDialogProps> = ({
  open,
  onClose,
  onSave,
  partner,
  ...
  const handleSearchPrograms = async (query: string) => {
    ...
    try {
      ...
      const filteredResults = allPrograms.filter(program => 
        ...
        program.shortDescription?.toLowerCase().includes(query.toLowerCase()) ||
        ...
        <Box sx={{ display: 'flex', height: '100%' }}>
          ...
          >
            ...
            <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
              Bạn có thể tìm kiếm các chương trình đào tạo hiện có để tham khảo hoặc sao chép.
            </Typography>
                    ...
                    <Typography variant="subtitle1" sx={{ fontWeight: 600, display: 'flex', alignItems: 'center', gap: 1 }}>
                      ...
                      {requestedProgram && (
                        <Chip
                          size="small"
                          label={
                            requestedProgram.status === 'PENDING' ? 'Chờ duyệt' : 
                            requestedProgram.status === 'APPROVED' ? 'Đã duyệt' : 'Từ chối'
                          }
                          color={
                            requestedProgram.status === 'PENDING' ? 'warning' : 
                            requestedProgram.status === 'APPROVED' ? 'success' : 'error'
                          }
                          sx={{
                  ...
                  <AccordionDetails sx={{ p: 3 }}>
                    ...
                    {requestedProgram && (
                      <Box sx={{ mb: 2 }}>
                        ...
                        >
                          <CardContent sx={{ p: 2 }}>
                            <Box sx={{ display: 'flex', justify: 'space-between', alignItems: 'flex-start', gap: 2 }}>
                              <Box sx={{ flex: 1 }}>
                                <Typography variant="body2" sx={{ fontWeight: 600, mb: 1 }}>
                                    ...
                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                                      ...
                                      <Box>
                                        ...
                                        <Typography variant="caption" color="text.secondary">
                                          {partner.representativeName} - {partner.position}
                                        </Typography>
                                      </Box>
                                    </Box>
                                  </Box>
                              ...
                              {requestedProgram.fileUrl && (
                                <Button
                                  size="small"
                                  variant="outlined"
                                  onClick={() => window.open(requestedProgram.fileUrl, '_blank')}
                                  sx={{ minWidth: 'auto' }}
                                >
                                  📎 File yêu cầu
                                </Button>
            ...
            <Box sx={{ mb: 3 }}>
              ...
              <Box sx={{ display: 'flex', gap: 2, mb: 2 }}>
                <TextField
                  ...
                  onChange={(e) => handleFormChange('listedPrice', parseInt(e.target.value) || 0)}
                />
                ...
                <TextField
                  ...
                  onChange={(e) => handleFormChange('internalPrice', parseInt(e.target.value) || 0)}
                />
              </Box>
              ...
              <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
                <TextField
                  ...
                  onChange={(e) => handleFormChange('publicPrice', parseInt(e.target.value) || 0)}
                />
