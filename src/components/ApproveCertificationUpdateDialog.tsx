...
const ApproveCertificationUpdateDialog: React.FC<ApproveCertificationUpdateDialogProps> = ({
  ...
  const comparisonRows = [
    ...
    { label: "Tên chứng chỉ", originalKey: "name", updateKey: "name" },
    { label: "Tổ chức cấp", originalKey: "issuedBy", updateKey: "issuedBy" },
    { label: "Ngày cấp", originalKey: "issueDate", updateKey: "issueDate" },
    { label: "Ngày hết hạn", originalKey: "expiryDate", updateKey: "expiryDate" },
    { label: "Trình độ", originalKey: "level", updateKey: "level" },
    { label: "URL tài liệu", originalKey: "certificateUrl", updateKey: "certificateUrl" },
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      if (confirmDialog.type === "approve") {
        await API.admin.approveCertificationUpdate({ id: data.content.update.id });
        toast.success("Phê duyệt cập nhật chứng chỉ thành công!");
      } else if (confirmDialog.type === "reject") {
        if (!adminNote.trim()) {
          toast.error("Vui lòng nhập lý do từ chối!");
          return;
        }
        await API.admin.rejectCertificationUpdate({
          id: data.content.update.id,
          adminNote: adminNote.trim(),
        });
        toast.success("Từ chối cập nhật chứng chỉ thành công!");
      }
        ...
        <DialogTitle>
          ...
          >
            <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
              ...
              <Typography variant="h6">Phê duyệt cập nhật chứng chỉ</Typography>
