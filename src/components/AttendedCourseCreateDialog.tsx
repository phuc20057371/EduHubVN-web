...
import { getVietnameseScale } from "../utils/ChangeText";
...
const AttendedCourseCreateDialog: React.FC<AttendedCourseCreateDialogProps> = ({
  open,
  data,
  onClose,
}) => {
  ...
  const handleApprove = useCallback(async () => {
    setLoading(true);
    try {
      const res = await API.admin.approveAttendedCourse({
        id: data.content.id,
      });
      if (!res.data.success) {
  ...
  const handleReject = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await API.admin.rejectAttendedCourse({
        id: data.content.id,
        adminNote,
      });
      if (!res.data.success) {
            ...
            <Stack spacing={3}>
              ...
              >
                <CardContent sx={{ p: 3 }}>
                  ...
                  >
                    <Chip
                      label={getVietnameseScale(content.scale)}
                      size="small"
                      sx={{
...
export default AttendedCourseCreateDialog;
